...
(function () {
  ...
  /* ================= 建议库 ================= */
  var ADVICE = {
    parkour:   { for: '爱跑酷',  color: '#7BA87B', text: '利用垂直空间，安装通顶猫爬架或墙面跳板，释放地面面积；跑酷动线尽量避免正对玻璃窗和脆弱摆件。', short: '通顶猫爬架/墙面跳板，跑酷动线避开玻璃窗。' },
    high:      { for: '喜欢高处', color: '#7BA87B', text: '在墙面错落安装 2-3 块承重跳板（间距≤40cm），或利用衣柜顶部打造制高点，猫从高处俯瞰领地会更有安全感。', short: '墙面装 2-3 块承重跳板（间距≤40cm），留制高点。' },
    peeing:    { for: '玻璃胃/乱尿', color: '#E05A5A', text: '避开动线死角，预留带排气扇的柜体空间放置猫砂盆；数量建议＝猫数+1，分布在彼此听得见但闻得到的位置。', short: '猫砂盆进带排气的柜体，数量=猫数+1，避开死角。' },
    destroyer: { for: '破坏王',  color: '#E05A5A', text: '沙发优选猫抓布/科技布，墙角和门框贴剑麻贴片；与其对抗天性，不如在它最爱挠的位置旁边立一个更舒服的猫抓柱。', short: '沙发选猫抓布，墙角贴剑麻，旁边立更舒服的猫抓柱。' },
    clingy:    { for: '粘人精',  color: '#FF9E7D', text: '在厨房、书桌旁预留人猫共享位——吧台边一块 30cm 深的猫台，让它守着你又不挡道。', short: '书桌/吧台旁留 30cm 猫台，守着你又不挡道。' },
    alone:     { for: '独处型',  color: '#FF9E7D', text: '给主子留一处视线好又隐蔽的猫洞（书柜挖格/床底通道），独处型的猫需要的不是热闹，是角落。', short: '留一处视线好又隐蔽的猫洞：书柜挖格或床底通道。' }
  };
  var PAIN_ADVICE = {
    window:    { for: '封窗安全', color: '#E05A5A', text: '必须使用金刚网纱窗（目数≤1.0cm）并做防撞加固，四角用膨胀螺丝固定；推拉窗加装限位器，开口≤10cm。', short: '必装金刚网纱窗+限位器（开口≤10cm），螺丝固定。' },
    furniture: { for: '家具保护', color: '#E05A5A', text: '浅色系选猫抓布、深色系选雪尼尔，都能扛爪；真皮慎入。家具腿包剑麻带，每隔 2m 设一个合法抓挠点。', short: '猫抓布沙发+剑麻贴片，每 2m 设一个合法抓挠点。' },
    litter:    { for: '猫砂盆布局', color: '#E05A5A', text: '优先做浴室半分区或阳台家政柜预留位：柜内放盆、侧面开 20cm 圆洞、顶部装排气扇。别挨着洗衣机——震动会让猫拒绝如厕。', short: '家政柜放盆：侧开 20cm 圆洞、顶装排气扇，远离洗衣机。' },
    aesthetic: { for: '人猫共居美学', color: '#FF9E7D', text: '走嵌入式路线：爬架换墙面木质跳板、猫窝藏进书柜格、猫砂盆进柜体，猫用品全用哑光木色/奶油色，水墨感就保住了。', short: '嵌入式路线：跳板/猫窝/猫砂盆全藏进柜体与墙面。' }
  };
  /* 居住情况派生建议（依据：猫听觉约为人 6 倍/噪音应激源；Fel d 1 蛋白附着织物，
     ISFM 环境控制清单：硬质地面、少布艺、HEPA、卧室禁区；独处 8h+ 行为问题率上升） */
  var LIVE_ADVICE = {
    alone:  { for: '独居陪伴', color: '#FF9E7D', text: '一个人养猫，上班时间的空白靠环境填：窗边观景位 + 漏食玩具 + 藏食点轮流上岗，回家后固定 15 分钟逗猫棒时间。', short: '窗景观赏位+漏食玩具+藏食点，回家固定逗猫 15 分钟。' },
    family: { for: '多人家庭', color: '#FF9E7D', text: '猫的听觉约为人的 6 倍，电视声和孩子哭闹都是应激源；在客厅高处留一个「退路位」，并全家统一规则——能不能上床，一个口径。', short: '客厅高处留退路位，全家统一养猫规则。' },
    share:  { for: '合租改造', color: '#FF9E7D', text: '租房大概率不能打孔：垂直空间用顶天立地猫柱、免钉层板和衣柜顶动线替代墙面跳板，搬家可带走；动工前先拿到室友同意。', short: '免打孔方案：顶天立地柱、免钉层板，搬家可带走。' }
  };
  var KIDS_ADVICE = { for: '儿童与猫共处', color: '#E05A5A', text: '婴儿房设为猫禁区，婴儿床装防护网；教孩子摸背不拽尾、猫吃饭睡觉时不打扰；猫砂盆放在孩子够不到的分区，铲屎后洗手——弓形虫和猫抓病都防在这一步。', short: '婴儿房设猫禁区+床防护网，猫砂盆放孩子够不到处。' };
  var ALLERGY_ADVICE = { for: '防过敏选材', color: '#7BA87B', text: '致敏的是 Fel d 1 蛋白不是猫毛：地面选木地板/瓷砖慎用地毯，沙发选易清洁的猫抓布，少用厚布艺窗帘，预留空气净化器电位，过敏者卧室设猫禁区。', short: '木地板替代地毯、少布艺、留净化器电位、卧室设禁区。' };
  var FALLBACK = [
    { for: '动线安全', color: '#7BA87B', text: '提前规划猫高速路：从窗户到制高点的路线避开过道正上方，人走人的路，猫飞猫的桥，互不打扰。', short: '猫动线避开过道正上方，人猫各行其道。' },
    { for: '水电收纳', color: '#7BA87B', text: '裸露电线全部入槽或缠麻绳防啃，插座选带防溅盖的款式——好奇猫的舌头比你想的离插座更近。', short: '电线入槽、插座加防溅盖。' },
    { for: '绿植预警', color: '#7BA87B', text: '装修后添绿植避开百合、绿萝、滴水观音等对猫有毒品种，选猫草、散尾葵、波士顿蕨更稳妥。', short: '避开百合、绿萝等毒植，选猫草、散尾葵。' }
  ];

  /* ===== 八项现状测评的建议条目 =====
     封窗：和猫住/猫德学院等救助机构领养审核的最低标准（金刚网，一镜到底敲窗视频核验） */
  var WINDOW_NET = { for: '封窗安全', tag: '必做', color: '#E05A5A', text: '金刚网纱窗是救助机构领养审核的最低标准：网孔≤5cm、四角膨胀螺丝固定；推拉窗加装限位器，开口≤10cm。普通纱窗猫一抓就破，等于没封。', short: '金刚网纱窗+限位器（开口≤10cm），螺丝固定——领养底线。' };
  /* 多猫：ISFM 的 N+1 原则（猫砂盆=猫数+1）+ 垂直分层减少领地冲突 */
  var MULTI = { for: '多猫家庭', color: '#FF9E7D', text: '猫砂盆数量＝猫数+1，分散布置不聚堆，离食水≥2m；每只猫至少 1 个专属高处点（2-3 个更稳），垂直分层能显著减少领地摩擦。', short: '猫砂盆=猫数+1，分散布置，离食水≥2m，每猫一个高处点。' };
  /* 危险绿植：百合花粉即可致急性肾衰；天南星科（绿萝/龟背竹/滴水观音）草酸钙 */
  var PLANT = { for: '危险绿植/阳台', color: '#E05A5A', text: '百合剧毒（花粉都可能致肾衰），绿萝、滴水观音等天南星科也别养；未封阳台的坠楼风险与未封窗同级，加装隔离网或改封闭式猫窗台。', short: '百合/绿萝/滴水观音别养；未封阳台加装隔离网。' };
  /* 陪伴不足：独处 8h+ 行为问题率显著上升（间歇性厌食/破坏行为/分离焦虑） */
  var COMPANY = { for: '陪伴不足预案', color: '#FF9E7D', text: '上班空白时间靠环境补：漏食玩具、藏食点、自动逗猫棒轮流上岗，窗边观景位是性价比最高的「猫电视」。', short: '漏食玩具+藏食点+自动逗猫棒，窗边观景位当猫电视。' };
  /* 最爱呆哪：行为学共识「顺应猫已选的路径布置，成功率远高于另起炉灶」 */
  var SPOT_ADVICE = {
    window: { for: '窗边黄金位', color: '#7BA87B', text: '窗边是它已选中的黄金位：做一个封窗后可躺靠的加宽窗台或吊板，宽度≥30cm、承重足够；别在窗边放它够得着的易倒摆件。', short: '加宽窗台/吊板（宽≥30cm），封窗后安心躺靠。' },
    high:   { for: '制高点动线', color: '#7BA87B', text: '顺应它的制高点偏好：从现有家具顶（衣柜/书柜）延伸 2-3 块承重跳板，终点留一个有靠背的观景台。', short: '从家具顶延伸 2-3 块跳板，终点留靠背观景台。' },
    hide:   { for: '隐蔽安全屋', color: '#FF9E7D', text: '床底是它的安全屋：别强行封堵，在动线附近给个同类替代——半封闭猫窝或书柜挖格，位置选在能看见全家动静的角落。', short: '别封床底，附近放半封闭猫窝或书柜挖格。' },
    human:  { for: '人猫共享位', color: '#FF9E7D', text: '它选了你当制高点：书桌/沙发旁留 30cm 深的共享位，比单独买猫窝成功率高得多；一块桌面延长板就够。', short: '书桌/沙发旁留 30cm 共享位，胜过单独猫窝。' }
  };
  /* 预算三档：入门 <1k 解决 80% 痛点；1k-5k 定制承重件；5k+ 整体木作/catio */
  var BUDGET_ADVICE = {
    low:  { for: '低预算路线', color: '#7BA87B', text: '千元内也能做 80 分：免打孔层板+顶天立地柱解决垂直空间，纸箱 DIY 藏食点——先把安全（封窗）做满，再谈升级。', short: '免打孔层板+顶天立地柱+纸箱藏食，先满封窗。' },
    mid:  { for: '标准预算路线', color: '#7BA87B', text: '1k-5k 是舒适区：金刚网整窗 + 2-3 块定制承重跳板 + 带排气扇的猫砂柜，安全与颜值可以兼得。', short: '金刚网整窗+承重跳板+排气猫砂柜。' },
    high: { for: '高预算路线', color: '#7BA87B', text: '5k+ 可以走定制：通顶猫爬架、嵌入式猫家具一体化木作，甚至封个 catio 猫阳台——动工前把猫动线画进装修图，比完工后补装省一半钱。', short: '通顶爬架/嵌入式木作，动线画进装修图再动工。' }
  };
  var VERT_NO = { for: '垂直空间', color: '#7BA87B', text: '还没给猫留爬高路线：墙面跳板间距 20-40cm 错落上行（幼猫老年猫取小值），或用顶天立地猫柱起步，承重件务必打进承重墙。', short: '跳板间距 20-40cm 错落上行，承重件打进承重墙。' };
  var DUPLEX = { for: '复式/Loft 动线', color: '#7BA87B', text: '复式住宅资源要每层配齐：猫砂盆、水碗每层各一份，楼梯口加防坠落挡板；护栏缝隙先量一量，猫能钻过去就能掉下去。', short: '猫砂盆水碗每层一份，楼梯口加防坠落挡板。' };
  var SMALL = { for: '小户型垂直化', color: '#7BA87B', text: '30㎡ 以内别跟地面较劲，把面积往墙上要：通顶猫柱+窗台延长板+门上跳台，地面只留猫砂盆和食水三个必占点。', short: '面积往墙上要：通顶柱+窗台板+门上跳台。' };
